...
// CHANGE (dcp) added GAP_CRIT, FIXED_CRIT, MAXCHANGE_CRIT, AVECHANGE_CRIT
#define GAP_CRIT           ((mask) 1 << 14)
#define FIXED_CRIT         ((mask) 1 << 15)
#define MAXCHANGE_CRIT     ((mask) 1 << 16)
#define AVECHANGE_CRIT     ((mask) 1 << 17)
#define CRITERION_MASKS    (STANDARD_CRIT | MAX_CRIT | GAP_CRIT | FIXED_CRIT | MAXCHANGE_CRIT | AVECHANGE_CRIT)
...
// dcp
#define DOT_PRODUCT_ONCE   ((mask) 1 << 11)

#define BASIC_INPUT_TYPES  (DOT_PRODUCT | DISTANCE | PRODUCT | IN_BOLTZ | \
			    IN_COPY | DOT_PRODUCT_ONCE)
#define IN_INTEGR_TYPES (IN_INTEGR | DOT_PRODUCT_ONCE)
...
// dcp
#define LOGISTIC_ONCE      ((mask) 1 << 21)
#define OUT_DECAY          ((mask) 1 << 22)

//sl
#define OUT_DECAY_TIME     ((mask) 1 << 23)
#define URGENCY	  	   ((mask) 1 << 24)
#define INV_SQUARE_DECAY   ((mask) 1 << 25)
#define ELBOW		   ((mask) 1 << 26)
#define OUT_DECAY_EXP	   ((mask) 1 << 27)
#define OUT_IPSP	   ((mask) 1 << 28)
#define OUT_IPSPADD	   ((mask) 1 << 29)
#define IPSP_DYNAMIC	   ((mask) 1 << 30)
#define IPSP_DYNAMICADD	   ((mask) 1 << 31)

#define BASIC_OUTPUT_TYPES (LINEAR | LOGISTIC | TERNARY | TANH | GAUSSIAN | \
                            EXPONENTIAL | SOFT_MAX | KOHONEN | OUT_BOLTZ | \
                            OUT_COPY | INTERACT_INTEGR | LOGISTIC_ONCE | ELBOW)
#define CLAMPING_OUTPUT_TYPES (HARD_CLAMP | BIAS_CLAMP | ELMAN_CLAMP | \
			       WEAK_CLAMP)
#define OUT_INTEGR_TYPES (OUT_INTEGR | INTERACT_INTEGR | LOGISTIC_ONCE)
