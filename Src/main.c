...
int Tcl_AppInit(Tcl_Interp *interp) {
  ...
  sprintf(Version, "%s%s/a", VERSION, VersionExt);
#else
...
#else
  sprintf(Buffer, "TCL_LIBRARY=%s/TclTk/tcl%s/library", RootDir, TCLVER);
  if (putenv(copyString(Buffer))) fatalError("couldn't set %s", Buffer);
  sprintf(Buffer, "TK_LIBRARY=%s/TclTk/tk%s/library", RootDir, TCLVER);
  if (putenv(copyString(Buffer))) fatalError("couldn't set %s", Buffer);
#endif /* MACHINE_WINDOWS */
  initializeSimulator();
