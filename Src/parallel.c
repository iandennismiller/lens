...
static flag sendEval(Tcl_Channel channel, char *message) {
...
static flag sendClientWeights(Client C) {
  ...
  FOR_EACH_GROUP({
	...
	if (i == PAR_BUF_SIZE) {
	  if (Tcl_Write(channel, (char *) buffer, blockSize)
  ...
  if (i && (Tcl_Write(channel, (char *) buffer, i * sizeof(int)) 
	    != i * sizeof(int)))
    return deleteClient(C);
...
static flag sendClientTrain(Client C, int randExample, int batchSize) {
  if (sendChar(C->channel, TRAIN) ||
      sendInt(C->channel, Net->numLinks) ||
      sendInt(C->channel, batchSize) ||
      sendInt(C->channel, randExample)) return deleteClient(C);
...
static int performUpdate(Client C, flag groupCritReached) {
  flag willReport;
  UpdatesDone++;
  Net->totalUpdates++;
  ...
  RUN_PROC(postUpdateProc); return(0) ;
}

#define realTime(i) (((real)i)/1000.0)

static void synchronousContinue(Client C, flag groupCritReached) {
  static flag groupCritsReached = TRUE;
  int active, totalBatch, desiredBatch, curEx, curExSend;
  ...
  if (Clients[IS_TRAINING] == 0) {
	...
	Net->error > Net->criterion) {
      ...
      // dcp
      if (Net->incrementalNumIncorrect &&
	  Net->numExamplesCorrect+Net->incrementalNumIncorrect > Net->batchSize && 
	  Net->batchSize < Net->trainingSet->numExamples) {
	Net->batchSize = MIN(Net->numExamplesCorrect+Net->incrementalNumIncorrect,Net->trainingSet->numExamples) ;
	print(1,"*** epoch %d batchSize %d\n", Net->totalUpdates, Net->batchSize) ;
      }

      desiredBatch = (Net->batchSize) ? Net->batchSize : 
	Net->trainingSet->numExamples;

      // dcp: scale batchSize by parallelBatchScaling (for PERMUTED example sets)
      desiredBatch = roundf(desiredBatch * Net->parallelBatchScaling) ;
	...
	C->batchSize = roundr(desiredBatch * C->proportion * 
			     totalExamplePerTime);
	if (Net->parallelSplit) 
	  while (C->batchSize % Net->parallelSplit) C->batchSize++ ;
	totalBatch += C->batchSize;
      });
      /* Make sure we haven't lost any */
      lastClient->batchSize += desiredBatch - totalBatch;

      curEx = 0 ;
      FOR_EACH_WAITING_CLIENT({
	if (C->batchSize == 0) continue;
	if (sendClientWeights(C)) continue;
	// dcp: if PERMUTED send random integer (to be used as seed by client)
	curExSend = (Net->trainingSet->mode & PERMUTED ? ((int)random()) : curEx) ;

	if (sendClientTrain(C, curExSend, C->batchSize)) continue;
	C->currentExampleNum = curEx ;
	curEx += C->batchSize ;
	// dcp
	// print(1,"%s setting curEx = %d with batchSize = %d [numCorr %d/%d]\n", C->hostName, C->currentExampleNum, C->batchSize, C->numExamplesCorrect, Net->numExamplesCorrect) ;
      });
      if (TestInterval && (UpdatesDone % TestInterval) == 0)
	performTest();
      groupCritsReached = TRUE;
      Net->error = 0.0;
      Net->numExamplesCorrect = 0;
...
static void asynchronousContinue(Client C, flag groupCritReached) {
  ...
  if ((UpdatesDone < Net->numUpdates) && !groupCritReached && 
    ...
    } else if (sendClientTrain(C, NORANDEX, Net->batchSize)) return;
  } else {
    if (UpdatesDone >= Net->numUpdates) 
      HaltCondition = ENOUGH_UPDATES;
...
static flag receiveClientDerivs(Client C) {
  ...
  if (receiveReal(C->channel, &err) ||
      ...
      receiveInt(C->channel, &C->numExamplesCorrect) ||
      receiveInt(C->channel, &C->batchTime)) return deleteClient(C);
  
  if (Synchronous) {
    Net->error += err;
    Net->numExamplesCorrect += C->numExamplesCorrect ;
  }
  else Net->error = err;
...
static flag receiveClientTestResults(Client C) {
  ...
  if (IsTraining) {
    print(1, "Performance after %d updates:\n%s\n", updates, Buffer);
    if (sendClientWeights(C) || sendClientTrain(C, NORANDEX, Net->batchSize))
      return TCL_ERROR;
  }
  return TCL_OK;
}
...
flag trainParallel(flag synch, int testInt) {
  ...
  if (Synchronous) {
    ...
    if (Net->parallelSplit) 
      while (size % Net->parallelSplit) size++ ;
    ...
    FOR_EACH_CLIENT({
      ...
      if (sendClientWeights(C)) continue;
      nClient++ ;
      if (nClient == NumClients)	// last client
	C->batchSize = batchSize - (size * (NumClients - 1)) ;
      else
	C->batchSize = size ;
      sendClientTrain(C, curEx, C->batchSize) ;
      C->currentExampleNum = curEx ;
      curEx += C->batchSize ;
    });
    ...
    FOR_EACH_CLIENT({
      ...
      else sendClientTrain(C, RANDEX, Net->batchSize);
    });
  }
...
static void acceptClient(ClientData junk, Tcl_Channel channel, char *hostName, 
  ...
  if (IsTraining) {
    ...
    if (!Synchronous) {
	  ...
	  sendClientTrain(C, RANDEX, Net->batchSize)) return;
    }
  }
...
static flag sendServerDerivs(flag groupCritReached, int numExamplesCorrect, unsigned int time) { 
  ...
  if (sendChar(ServerChannel, TRAIN) ||
      ...
      sendInt(ServerChannel, numExamplesCorrect) ||
  ...
  FOR_EACH_GROUP({
	...
	buffer[i++] = HTONL(*(int *) &v);
	if (i == PAR_BUF_SIZE) {
	  if (Tcl_Write(ServerChannel, (char *) buffer, blockSize) 
  ...
  if (i && (Tcl_Write(ServerChannel, (char *) buffer, i * sizeof(int)) 
...
static flag receiveServerTrain(void) {
  ...
  if (receiveInt(ServerChannel, &links) ||
      ...
      receiveInt(ServerChannel, &randExample)) return stopClient();
  ...
  */
  if (randExample == RANDEX) {
    Net->trainingSet->currentExampleNum = 
      randInt(Net->trainingSet->numExamples) - 1;
    Net->totalUpdates = 0;
    //    print(1,"randomizing currentExampleNum = %d\n", Net->trainingSet->currentExampleNum) ;
  } else if (randExample == NORANDEX) {
    //    print(1,"leaving currentExampleNum = %d unchanged\n",  Net->trainingSet->currentExampleNum) ;
    // do nothing
  } else if (Net->trainingSet->mode & PERMUTED) {
    // dcp: permute over entire example set independently on each client
    // PERMUTED example set - re-seed random number generator with "example number" sent by server
    // print(1,"reseeding with example num = %d\n", randExample) ;
    seedRand(randExample) ;
    Net->trainingSet->currentExampleNum = 0 ;
    Net->totalUpdates = 0;
  } else {	// set currentExampleNum
    Net->trainingSet->currentExampleNum = randExample ;
    print(1,"setting currentExampleNum = %d\n", Net->trainingSet->currentExampleNum) ;
    Net->totalUpdates = 0;
  }
  // end
  ...
  if (result == TCL_ERROR) {
    ...
    print(1, ".");
    time = getTime() - time;
    if (sendServerDerivs(groupCritReached, Net->numExamplesCorrect, time)) 
      result = TCL_ERROR;
  }
  return result;
}
