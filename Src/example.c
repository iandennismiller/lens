...
static flag readExampleSet(char *setName, char *fileName, ExampleSet *Sp, 
			   flag pipe, int maxExamples) {
  ExampleSet S = *Sp;
  Example E = NULL;
  ...
  if (!(R->channel = readChannel(fileName)))
...
flag loadExamples(char *setName, char *fileName, int mode, int numExamples) {
  ExampleSet S;
  ...
  if (!S || mode != 0)
    if (readExampleSet(setName, fileName, &S, (mode == 3), numExamples)) {
      deleteExampleSet(S);
      return TCL_ERROR;
    }
  if (Net && S) {
...
flag writeExampleFile(ExampleSet S, char *fileName, flag append) {
  int v, w;
  flag halted;
  Example E;
  Event V;
  ...
  if (!(channel = writeChannel(fileName, append)))
...
flag writeBinaryExampleFile(ExampleSet S, char *fileName) {
...
flag openNetOutputFile(char *fileName, flag binary, flag append) {
  Tcl_Channel channel;
  if (Net->outputFile) {
  ...
  if (!(Net->outputFile = channel = writeChannel(fileName, append)))
