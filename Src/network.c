...
static void buildUnitHistories(Group G, Unit U, char forceRebuild) {
  if (forceRebuild) {
    FREE(U->inputHistory);
    FREE(U->outputHistory);
    FREE(U->targetHistory);
...
static flag initUnit(Unit U, char *name, int num, Group G) {
  ...
  U->activeTicks   = DEF_U_activeTicks;
...
flag finalizeGroupType(Group G, flag wasOutput, flag wasInput) {
  ...
  FOR_EVERY_UNIT(G, buildUnitHistories(G, U, FALSE));
  return TCL_OK;
}

void changeGroupType(Group G, unsigned int class, mask type, 
		     unsigned int mode) {
  char hasType = GROUP_HAS_TYPE(G, class, type);
  if (mode == TOGGLE_TYPE)
    mode = (hasType) ? REMOVE_TYPE : ADD_TYPE;
  switch (mode) {
  case ADD_TYPE:
    if (!hasType) {
...
flag addGroup(char *name, int numTypes, unsigned int *typeClass, mask *type, 
  ...
  G->weightEliminationW0= DEF_G_weightEliminationW0;
  ...
  G->targetOneRadius    = DEF_G_targetRadius;
  G->targetZeroScaling  = DEF_G_targetZeroScaling;
  ...
  G->actDecayTime	= 0.0 ;
  G->decayConstant      = 0.0 ;
  G->decayNoiseRange	= 0.0 ;	
  G->invSquareTuning 	= 0.0 ;
  G->elbowPoint		= 0.5 ;
  G->maxTick		= 1.0 ;
  G->IPSPthreshold	= .5  ;
  ...
  // dcp: force bias group to have initOutput = 1.0
  G->initOutput         = (*type&BIAS ? 1.0 : DEF_G_initOutput) ;

  // dcp
  G->outputGain		= 1.0 ;

  G->initOutputRange       = DEF_G_initOutputRange;
...
flag addNet(char *name, mask type, int timeIntervals, 
  ...
  N->weightEliminationW0= DEF_N_weightEliminationW0;
  ...
  N->targetOneRadius    = DEF_N_targetRadius;
  N->targetZeroScaling = DEF_N_targetZeroScaling;
  N->zeroErrorRadius = DEF_N_zeroErrorRadius;

  // dcp
  N->incrementalNumIncorrect = 0 ;
  N->parallelBatchScaling    = 1.0 ;
  N->parallelBatchScaling    = 1.0 ;
  ...
  N->initOutputRange    = DEF_N_initOutputRange;
...
flag setTime(Network N, int timeIntervals, int ticksPerInterval,
  ...
  FOR_EACH_GROUP({
    FOR_EVERY_UNIT(G, buildUnitHistories(G, U, TRUE));
    if (initGroupTypes(G)) return TCL_ERROR;
  });
  return TCL_OK;
}
