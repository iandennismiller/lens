...
void storeOutputsAndTargets(int tick) {
  ...
  FOR_EACH_GROUP({
      // dcp (CONTINUOUS)
      //   forces tick 0 states to be stored so they are used during tick 1 backprop
      //      if (UnitUp || G->type & USE_OUTPUT_HIST)
      if (UnitUp || G->type & USE_OUTPUT_HIST || Net->type & CONTINUOUS)
      storeOutputs(G, tick);
...
void storeOutputDerivs(Group G, int tick) {
  ...
  // dcp (CONTINUOUS)
  //   seems like a hack, but it's needed for Lens and Xerion to agree....
  //  FOR_EACH_UNIT(G, U->outputDeriv *= 2.0) ;
...
void resetOutputs(Group G) {
  real *O = G->output, 
    // dcp (force bias initOutput
    initOutput = chooseValue(G->initOutput, Net->initOutput),
    // dcp (add random Gaussian noise to initial outputs)
    initOutputRange = chooseValue(G->initOutputRange, Net->initOutputRange);
  
  // dcp (CONTINUOUS)
  //	forces hard-clamped inputs to be set and stored for tick 0
  //  FOR_EACH_UNIT2(G, U->output = O[u] = initOutput);
  if (!G->inputProcs || G->outputType & HARD_CLAMP) {
    FOR_EACH_UNIT2(G, U->output = O[u] = (isNaN(U->externalInput) ? randReal(initOutput,initOutputRange) : U->externalInput));
  } else {
    FOR_EACH_UNIT2(G, U->output = O[u] = randReal(initOutput,initOutputRange));
  }
}
...
static void dotProductInputBack(Group G, GroupProc P) {
  FOR_EACH_UNIT(G, {
    ...
    FOR_EACH_LINK_FAST_BACK(U, {
      ...
      // dcp (print)
	DB(printf("%s: old %.6f iD %.6f act %.6f ",U->name,L_DRV,inputDeriv,V_OUT));
...
static void integrateInputBack(Group G, GroupProc P) {
  ...
  FOR_EACH_UNIT2(G, {
      ...
      // dcp (CONTINUOUS)
      lastInputDeriv[u] += dt * U->dtScale * (U->inputDeriv - lastInputDeriv[u]) ;
      U->inputDeriv = lastInputDeriv[u] ;
  });
}
...
//sl
static void elbowOutput(Group G, GroupProc P) {
	real *originalOutput = P->unitHistoryData[HISTORY_INDEX(Net->currentTick)];
	FOR_EACH_UNIT2(G,
	if (originalOutput[u] < G->elbowPoint){
		U->output = U->input;
		}
	else {
		U->output = 2 * U->input;
...
static void elbowOutputBack(Group G, GroupProc P){
	real *originalOutput = P->unitHistoryData[HISTORY_INDEX(Net->currentTick)];
	FOR_EACH_UNIT2(G,
		if (originalOutput[u] < G->elbowPoint)
		{ U->inputDeriv = U->outputDeriv;}
		else
		{U->inputDeriv = 2 *  U->outputDeriv;} 
	);
}


static void elbowOutputInit(Group G, GroupProc P) {
  P->forwardProc = elbowOutput;
  P->backwardProc = elbowOutputBack;
  P->unitHistoryData = realMatrix(Net->historyLength, G->numUnits,"ELBOW:unitHistoryData");

}

static void logisticOutput(Group G, GroupProc P) {
  // dcp
  real outputGain = chooseValue(G->outputGain, Net->outputGain);
  ...
  // dcp [CURRENTLY NOT ADJUSTING logisticOutputBack]
  //     implements a type of activation decay
  if (outputGain != 1.0) 
    FOR_EACH_UNIT(G, U->output *= outputGain) ;
}
...
static void logisticOutputBack(Group G, GroupProc P) {
  if (G->type & ADAPTIVE_GAIN) {
    ...
    // dcp (print)
    DB(FOR_EACH_UNIT(G, printf("%9.6f/%9.6f ", U->outputDeriv, U->inputDeriv)););
  }
}
...
static void integrateOutputBack(Group G, GroupProc P) {
  ...
  FOR_EACH_UNIT2(G, {
      ...
      // dcp (CONTINUOUS)
      lastOutputDeriv[u] += dt * U->dtScale * (U->outputDeriv - lastOutputDeriv[u]) ;
      U->outputDeriv = lastOutputDeriv[u] ;
      U->output = instantOutput[u];
  });
}
...
static void decayOutput(Group G, GroupProc P) {
  real *originalOutput = P->unitHistoryData[HISTORY_INDEX(Net->currentTick)];
  real decay = G->actDecay, rest = G->actRest ;
  FOR_EACH_UNIT2(G, {
    originalOutput[u] = U->output;
    U->output -= decay  * (U->output - rest) ;
  });
}

static void decayOutputTime(Group G, GroupProc P) {
	real *originalOutput = P->unitHistoryData[HISTORY_INDEX(Net->currentTick)];
	real rest = G->actRest ;
	real constant = G->decayConstant ;
	real decayRange = G->decayNoiseRange;
	real noiseAmount = randGaussian(0.0, decayRange);
	FOR_EACH_UNIT2(G, {
	  real minus = constant * Net->currentTick * (U->output - rest);
	  originalOutput[u] = U->output;
		U->output -= (minus + noiseAmount);
	});
}

static void decayOutputExp(Group G, GroupProc P) {
	real *originalOutput = P->unitHistoryData[HISTORY_INDEX(Net->currentTick)];
	real rest = G->actRest ;
	real constant = G->decayConstant ;
	FOR_EACH_UNIT2(G, {
		real minus = constant * exp(Net->currentTick) * (U->output - rest);
		originalOutput[u] = U->output;
		U->output -= minus;
	});
}

static void IPSP(Group G, GroupProc P){
	real *originalOutput = P->unitHistoryData[HISTORY_INDEX(Net->currentTick)];
	real rest = G->actRest ;
	real constant = G->decayConstant ;
	real maxTick = G->maxTick;
	real minusTick = Net->currentTick * -1;
	real num = minusTick / maxTick;
	FOR_EACH_UNIT2(G, {
		real times = constant * Net->currentTick * exp(num);
		originalOutput[u] = U->output;
		U->output = U->output * times; 
	});
}

static void IPSPadd(Group G, GroupProc P){
	real *originalOutput = P->unitHistoryData[HISTORY_INDEX(Net->currentTick)];
	real rest = G->actRest ;
	real constant = G->decayConstant ;
	real maxTick = G->maxTick;
	...
	real num = minusTick / maxTick;
	FOR_EACH_UNIT2(G, {
		real plus = constant * Net->currentTick * exp(num);
		originalOutput[u] = U->output;
		real potentialOutput = U->output + plus;
		U->output = potentialOutput;
	});
}

static void IPSPDynamic(Group G, GroupProc P){
	real *originalOutput = P->unitHistoryData[HISTORY_INDEX(Net->currentTick)];
	real constant = G->decayConstant;
	real maxTick = G->maxTick;
	FOR_EACH_UNIT2(G, {
		if (Net->currentTick == 1){
			U->activeTicks = 1;
			//print(1, "%d %d \n", Net->currentTick, U->activeTicks);
		}
		real aTicks = (float) U->activeTicks;
		real minusTick = (float) U->activeTicks * -1;
		real num = (float) minusTick / maxTick;
		real times = (float) constant * aTicks * exp(num);
		originalOutput[u] = U->output;
		U->output = U->output * times;
		//print(1, "%f\n", U->output);
		real IPSPOut = U->output;
		real threshold = G->IPSPthreshold;
		//print(1, "%f %f \n", IPSPOut, threshold);
		if (IPSPOut > threshold) {
			aTicks++;
			U->activeTicks = aTicks;
			//print(1, "%f %d \n", aTicks, U->activeTicks);
		}
	});
}

static void IPSPDynamicAdd(Group G, GroupProc P){
        real *originalOutput = P->unitHistoryData[HISTORY_INDEX(Net->currentTick)];
        real constant = Net->decayConstant;
        real maxTick = G->maxTick;
        FOR_EACH_UNIT2(G, {
                if (Net->currentTick == 1){
                        U->activeTicks = 1;
                ...
                real minusTick = (float) U->activeTicks * -1;
                real num = (float) minusTick / maxTick;
                real plus = (float) constant * aTicks * exp(num);
                originalOutput[u] = U->output;
                U->output = U->output + plus;
                //print(1, "%f\n", U->output);
                real IPSPOut = U->output;
                real threshold = Net->IPSPthreshold;
                //print(1, "%f %f \n", IPSPOut, threshold);
                if (IPSPOut > threshold) {
                        aTicks++;
                        U->activeTicks = aTicks;
...
static void urgency(Group G, GroupProc P) {
	real *originalOutput = P->unitHistoryData[HISTORY_INDEX(Net->currentTick)];
	real rest = G->actRest;
	real constant = G->decayConstant;
	FOR_EACH_UNIT2(G, {
          int tick = Net->currentTick;
          if (tick <= 10)
          {
          real minus = constant * Net->currentTick * (U->output - rest);
          originalOutput[u] = U->output;
          ...
          {
          real minus = constant * 10 * (U->output - rest);
          originalOutput[u] = U->output;
                U->output -= (minus);
          }
        });
...
static void invSquareDecay(Group G, GroupProc P) {
	real *originalOutput = P->unitHistoryData[HISTORY_INDEX(Net->currentTick)]
;
	real rest = G->actRest;
	FOR_EACH_UNIT2(G, {
		real invSquare = abs(1/pow(U->output,.5));
		if (U->output > 0)
		{
		real minus = G->invSquareTuning * invSquare * (U->output - rest);
		originalOutput[u] = U->output;
		real potentialOutput = U->output - minus;
...
static void decayOutputBack(Group G, GroupProc P) {
  real *originalOutput = P->unitHistoryData[HISTORY_INDEX(Net->currentTick)];
  FOR_EACH_UNIT2(G, U->output = originalOutput[u]);
}

static void decayOutputTimeBack(Group G, GroupProc P) {
	real *originalOutput = P->unitHistoryData[HISTORY_INDEX(Net->currentTick)];
	FOR_EACH_UNIT2(G, U->output = originalOutput[u]);
}

static void decayOutputExpBack(Group G, GroupProc P) {
	real *originalOutput = P->unitHistoryData[HISTORY_INDEX(Net->currentTick)];
	FOR_EACH_UNIT2(G, U->output = originalOutput[u]);
}

static void IPSPBack(Group G, GroupProc P) {
	real *originalOutput = P->unitHistoryData[HISTORY_INDEX(Net->currentTick)];
        FOR_EACH_UNIT2(G, U->output = originalOutput[u]);
}

static void IPSPaddBack(Group G, GroupProc P) {
	real *originalOutput = P->unitHistoryData[HISTORY_INDEX(Net->currentTick)];
	FOR_EACH_UNIT2(G, U->output = originalOutput[u]);
}

static void IPSPDynamicBack(Group G, GroupProc P){
	real *originalOutput = P->unitHistoryData[HISTORY_INDEX(Net->currentTick)];
	FOR_EACH_UNIT2(G, U->output = originalOutput[u]);
}

static void IPSPDynamicAddBack(Group G, GroupProc P){
        real *originalOutput = P->unitHistoryData[HISTORY_INDEX(Net->currentTick)];
        FOR_EACH_UNIT2(G, U->output = originalOutput[u]);
}

static void urgencyBack(Group G, GroupProc P) {
	real *originalOutput = P->unitHistoryData[HISTORY_INDEX(Net->currentTick)];
	FOR_EACH_UNIT2(G, U->output = originalOutput[u]);
}

static void invSquareDecayBack(Group G, GroupProc P) {
	real *originalOutput = P->unitHistoryData[HISTORY_INDEX(Net->currentTick)];
	FOR_EACH_UNIT2(G, U->output = originalOutput[u]);
}

static void decayOutputInit(Group G, GroupProc P) {
  P->forwardProc  = decayOutput;
  P->backwardProc = decayOutputBack;
  P->unitHistoryData = realMatrix(Net->historyLength, G->numUnits,
				  "OUT_DECAY:unitHistoryData");
}

static void decayOutputTimeInit(Group G, GroupProc P) {
	P->forwardProc = decayOutputTime;
	P->backwardProc = decayOutputTimeBack;
	P->unitHistoryData = realMatrix(Net->historyLength, G->numUnits,
				"OUT_DECAY_TIME:unitHistoryData");
}

static void decayOutputExpInit(Group G, GroupProc P){
	P->forwardProc = decayOutputExp;
	P->backwardProc = decayOutputExpBack;
	P->unitHistoryData = realMatrix(Net->historyLength, G->numUnits,
				"OUT_DECAY_EXP:unitHistoryData");
}

static void IPSPInit(Group G, GroupProc P){
	P->forwardProc = IPSP;
	P->backwardProc = IPSPBack;
	P->unitHistoryData = realMatrix(Net->historyLength, G->numUnits, 
				"OUT_IPSP:unitHistoryData");
}

static void IPSPaddInit(Group G, GroupProc P){
	P->forwardProc = IPSPadd;
	P->backwardProc = IPSPaddBack;
	P->unitHistoryData = realMatrix(Net->historyLength, G->numUnits,
				"OUT_IPSPADD:unitHistoryData");
}

static void IPSPDynamicInit(Group G, GroupProc P){
	P->forwardProc = IPSPDynamic;
	P->backwardProc = IPSPDynamicBack;
	P->unitHistoryData = realMatrix(Net->historyLength, G->numUnits,
				"IPSP_DYNAMIC:unitHistoryData");
}

static void IPSPDynamicAddInit(Group G, GroupProc P){
	P->forwardProc = IPSPDynamicAdd;
	P->backwardProc = IPSPDynamicAddBack;
	P->unitHistoryData = realMatrix(Net->historyLength, G->numUnits,
		"IPSP_DYNAMICADD:unitHistoryData");
}

static void urgencyInit(Group G, GroupProc P){
	P->forwardProc = urgency;
	P->backwardProc = urgencyBack;
	P->unitHistoryData = realMatrix(Net->historyLength, G->numUnits,
				"URGENCY:unitHistoryData");
}

static void invSquareDecayInit(Group G, GroupProc P){
	P->forwardProc = invSquareDecay;
	P->backwardProc = invSquareDecayBack;
	P->unitHistoryData = realMatrix(Net->historyLength, G->numUnits,
				"INV_SQUARE_DECAY:unitHistoryData");
}
...
static void squaredError(Group G, GroupProc P) {
  ...
  if (targetRadius != 0.0 || targetOneRadius != 0.0 || zeroErrorRadius != 0.0 || targetZeroScaling != 1.0) {
	...
	unitError = SQUARE(output - target);
...
static void squaredErrorBack(Group G, GroupProc P) {
  real targetZeroScaling = chooseValue(G->targetZeroScaling, Net->targetZeroScaling) ;
  real scale = ((Net->pseudoExampleFreq) ? 
		Net->currentExample->frequency : 1.0) * G->errorScale * 2.0;
  real zeroTargetScale = scale * targetZeroScaling ;
  if (targetZeroScaling != 1.0) {
    FOR_EACH_UNIT(G, {
      if (!isNaN(U->target)) {
	  U->outputDeriv += 
	    (U->output - U->adjustedTarget) * 
	    (U->target == 0.0 ? zeroTargetScale : scale) ;
      }
    });
  } else {
    FOR_EACH_UNIT(G, {
...
static void crossEntropyErrorBack(Group G, GroupProc P) {
  real output, target, 
    scale = ((Net->pseudoExampleFreq) ? Net->currentExample->frequency :
	     1.0) * G->errorScale, 
    targetZeroScaling = chooseValue(G->targetZeroScaling, Net->targetZeroScaling),
    zeroTargetScale = scale * targetZeroScaling ;
  if (targetZeroScaling != 1.0) {
    FOR_EACH_UNIT(G, {
	if (!isNaN(U->target)) {
	  output = U->output; target = U->adjustedTarget;
	  U->outputDeriv += 
	    CROSS_ENTROPY_DERIV(output, target) * 
	    (U->target == 0.0 ? zeroTargetScale : scale) ;
	}
    });
  } else {
...
void computeCost(Group G, flag alwaysStore) {
  GroupProc P;
  // dcp
  int inGrace = (Net->inGracePeriod || Net->currentTick <= G->graceTick) ;
  if (inGrace) {
...
void computeCostBack(Group G, flag alwaysStore) {
  ...
  /* Do the backward procedures in reverse order. */
  if (G->costProcs) {
    if (!inGrace && (P = G->costProcs->prev)) do {
      if (P->backwardProc) P->backwardProc(G, P);
      P = P->prev;
...
flag continuousNetExampleBack(Example E) {
  ...
  // dcp (print)
  DB(FOR_EACH_GROUP(FOR_EACH_UNIT(G,FOR_EACH_LINK_FORW(U,printf("%.6f ",G->net->dt*L->deriv))));
     printf("\n"););
...
flag standardNetTrainBatch(flag *allCorrect) {
  ...
  *allCorrect = TRUE;
  ...
  for (i = 0, Net->numExamplesCorrect = 0; i < numExamples; i++) {
...
flag standardNetTestBatch(int numExamples, flag resetError, 
                          flag printResults) {
  int i, set, totalTicks, reached = 0;
  ExampleSet S;
  Example E;
...
flag boltzmannNetTrainExample(Example E, flag (*tickProc)(Event), 
  ...
  flag phaseDone;
  real timeOnPhase, timeOnEvent, minTime = 0.0, maxTime = 0.0, graceTime = 0.0;
  Event V = NULL;
...
void registerNetAndGroupTypes(void) {
  ...
  registerGroupType("LINEAR", LINEAR, GROUP_OUTPUT,
		    linearOutputInit);
  //sl
  registerGroupType("ELBOW", ELBOW, GROUP_OUTPUT, elbowOutputInit);
  ...
  registerGroupType("OUT_DECAY", OUT_DECAY, GROUP_OUTPUT,
		    decayOutputInit);
  registerGroupType("INTERACT_INTEGR", INTERACT_INTEGR, GROUP_OUTPUT,
		    interactiveActivationInit);
  //sl
  registerGroupType("OUT_DECAY_TIME", OUT_DECAY_TIME, GROUP_OUTPUT, 
			decayOutputTimeInit);
  registerGroupType("URGENCY", URGENCY, GROUP_OUTPUT,
			urgencyInit);
  registerGroupType("INV_SQUARE_DECAY", INV_SQUARE_DECAY, GROUP_OUTPUT,
			invSquareDecayInit);
  registerGroupType("OUT_DECAY_EXP", OUT_DECAY_EXP, GROUP_OUTPUT,
			decayOutputExpInit);
  registerGroupType("OUT_IPSP", OUT_IPSP, GROUP_OUTPUT,
			IPSPInit);
  registerGroupType("OUT_IPSPADD", OUT_IPSPADD, GROUP_OUTPUT,
			IPSPaddInit);
  registerGroupType("IPSP_DYNAMIC", IPSP_DYNAMIC, GROUP_OUTPUT,
			IPSPDynamicInit);
  registerGroupType("IPSP_DYNAMICADD", IPSP_DYNAMICADD, GROUP_OUTPUT,
	IPSPDynamicAddInit);
