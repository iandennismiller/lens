...
static int C_time(TCL_CMDARGS) {
  ...
  return result("%.3f active  %.3f user  %.3f system  %.3f real",
		(real) (userticks + systicks) / CLK_TCK,
		(real) userticks / CLK_TCK,
		(real) systicks / CLK_TCK,
		(real) realticks / CLK_TCK);
}
...
extern void TkConsolePrint (Tcl_Interp *interp,	int devId, 
...
flag createConsole(flag firstTime) {
  InitConsoleChannels(Interp);
  if (CreateConsoleWindow(Interp))
